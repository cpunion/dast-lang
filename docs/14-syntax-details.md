...
- 所有整数类型均参与：`i8/i16/i32/i64`, `u8/u16/u32/u64`, `isize/usize`, `int`, `char`。
- 二元算术/位运算（`+ - * / % & | ^`）与比较（`== != < <= > >=`）使用 **Peer Type**：
  - 计算两个操作数类型的**值域并集**（min/max）。
  - 选择**能覆盖该值域的最小整数类型**作为结果类型（比较返回 `bool`）。
  - 若 `int/isize/usize/char` 参与且其值域覆盖并集，则**优先返回该类型**。
- 字面量整数（编译期整数）可在**值域允许**时隐式收敛到对方类型；否则报错。
- 若无法找到可容纳并集的整数类型，则报错（例如 `i64 + u64` 在没有更宽有符号类型时不成立）。
...
- **bool**：仅允许 `&&` `||` `==` `!=`。
- **String / &String**：仅允许 `+`（拼接）与 `==/!=`（内容相等）。
- **enum**：仅允许 `==/!=` 且两侧必须为**同一枚举类型**。
- **char**：视为无符号 32 位整数，遵循整数 Peer Type 规则。
