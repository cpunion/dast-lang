...
| 11 | `&&` | 逻辑与 |
| 12 | `\|\|` | 逻辑或 |
| 13 | `..` `..=` | 范围（**仅用于模式匹配/for 语法，不是普通表达式运算符**） |
| 14 | `=` | 赋值（当前实现不支持复合赋值 `+=`/`-=` 等） |
...
- 若整数字面量在整个表达式链中仍未被约束，最终默认收敛为 `i64`。
...
- **数组字面量**：当期望类型为 `[T]` 时，元素按 `T` 进行隐式转换（例如字符串字面量可在 `[String]` 中使用）。未给出期望类型时，元素类型必须一致。

> 详见《类型系统》中的**隐式转换矩阵**（仅在上下文期望类型时生效）。
...
- 规则（对齐 Zig）：
  ...
  - 若浮点字面量在整个表达式链中仍未被约束，最终默认收敛为 `f64`；
  - `+ - * /` 允许，`%` 与位运算不允许；
  - 比较返回 `bool`。
...
> 说明：Stage2 当前不支持元组/数组模式（Stage0 支持）。
...
> **当前实现说明（Stage0/Stage2）**  
> - **or-pattern**：Stage0 禁止绑定；Stage2 要求所有分支的绑定名集合一致。  
> - **range-pattern**：仅支持 `int/char`。Stage0 起始必须为简单字面量或标识符；Stage2 起始/终止可为表达式，并支持 `..`/`..=`/开区间。  
