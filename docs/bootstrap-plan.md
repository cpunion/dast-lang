...
- **IR v0**：简化 IR，足够 bootstrap，支持解释器和 C codegen
- **Stage0/1/2** 均使用 IR v0

### 未来路线

参见 [docs/21-ir-design.md](21-ir-design.md) —— Rust 级能力的 IR 设计：

```
┌─────────────────────────────────────────────────────────┐
│                     IR 层次架构                         │
├─────────────────────────────────────────────────────────┤
│  AST → IR (高级) → IR v0 (低级) → Backend              │
│                                                         │
│  IR (高级): Rust MIR 级别                                │
│    - SSA 形式，Place/Operand                             │
│    - 显式 Move/Copy/Drop                                 │
│    - 泛型单态化                                           │
│    - Unwinding 支持                                       │
│                                                         │
